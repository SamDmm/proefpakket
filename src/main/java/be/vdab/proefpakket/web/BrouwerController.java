...
class BrouwerController {
	...
	ModelAndView proefpakketStap1NaarStap2(@PathVariable Optional<Brouwer> brouwer, @Validated(Bestelling.Stap1.class) Bestelling bestelling, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
		if (brouwer.isPresent()) {
			...
			return new ModelAndView(PROEFPAKKET_STAP_2_VIEW).addObject(brouwer.get()).addObject("gemeentes", gemeenteService.findAll());
		}
		redirectAttributes.addAttribute("fout", "Brouwer niet gevonden");
		return new ModelAndView(REDIRECT_BIJ_BROUWER_NIET_GEVONDEN);
	}
	@PostMapping(value = "{brouwer}/proefpakket", params = "stap1")
	ModelAndView proefpakketStap2NaarStap1(@PathVariable Optional<Brouwer> brouwer, Bestelling bestelling, RedirectAttributes redirectAttributes) {
		if (brouwer.isPresent()) {
			return new ModelAndView(PROEFPAKKET_STAP_1_VIEW).addObject(brouwer.get());
		}
		redirectAttributes.addAttribute("fout", "Brouwer niet gevonden");
	...
	ModelAndView proefpakketOpslaan(@PathVariable Optional<Brouwer> brouwer, @Validated(Bestelling.Stap2.class) Bestelling bestelling, BindingResult bindingResult, SessionStatus sessionStatus, RedirectAttributes redirectAttributes) {
		if (brouwer.isPresent()) {
			if (bindingResult.hasErrors()) {
